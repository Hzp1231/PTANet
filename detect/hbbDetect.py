...
def process_images(path, model):
    ...
    #     exit()
    images_path=path+'visible/val/images/'
    image_path=path+'infrared/val/images/'
    ...
    for   i in tqdm(range(len(all_file))):
        ...
        for pos, cls_value,cls_conf in zip(xywh_, cls_,class_conf_):
            ...
            cv2.imwrite("E:\\results\\baseline_add_vtuav\\rgb\\"+files, maskrgb)
            cv2.imwrite("E:\\results\\baseline_add_vtuav\\ir\\"+files, maskir)
        print(x)
...
def main():
    ...
    model = load_model("E:\detect\\baseline_Add_vtuav640\weights/best.pt", device)
    process_images("E:\VTUAV_downsample/", model)
